...
const LocalSnrObc = ({ graphOption, graphOptionValue, currentTab }) => {
  ...
  useEffect(() => {
    ...
    if (graphOption === 'live') {
       ...
       const setTime = setInterval(() => {
         const url = 'api/v2/troubleshoot/obc/trainlsnr';
 ...
 useEffect(() => {
    let url = '';
    if (graphOption === 'live') {
      url = 'api/v2/troubleshoot/obc/trainlsnr';
    }
    else {
        // if(currentTab === 'trainradio'){
            url =`api/v2/nodes/suradio/${nodeDataId}?${graphOptionValue}&att=lsnr,rsnr`;
        // }
    }
    getServerStatusDt(url);
