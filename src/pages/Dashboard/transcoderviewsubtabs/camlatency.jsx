...
const CamLatencyChart = ({ graphOption, graphOptionValue ,labelName }) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (graphOption === 'live') {
          let dt = new Date();
          const dataNew = {
            Latency: Number(parseFloat(data.data[labelName]).toFixed(2)) || 0,
  ...
  useEffect(() => {
    ...
    if (graphOption === 'live') {
      const setTime = setInterval(() => {
        const url =  `api/v2/troubleshoot/transcoder/${nodeIpaddress}/cameraping/${labelName}`;
...
export default CamLatencyChart;
